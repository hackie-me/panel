...
function Select-Directories {
    param([array]$AllDirectories)
    
    Write-ColorOutput "`nAvailable directories:" "Cyan"
    for ($i = 0; $i -lt $AllDirectories.Count; $i++) {
        $dir = $AllDirectories[$i]
        Write-ColorOutput "  [$i] $($dir.Name)" "White"
    }
    
    Write-ColorOutput "`nOptions: 'all', 'community', 'apis', or indices (0,1,2)" "Cyan"
    ...
    do {
        ...
        if ($selection -eq 'all') {
            ...
            try {
                ...
                Write-ColorOutput "Invalid selection. Please try again." "Red"
            }
        }
...
function Main {
    ...
    if ($WhatIf) {
        ...
        Write-ColorOutput "`nIMPORTANT: Close Visual Studio before proceeding!" "Red"
    ...
    $selectedDirectories = Select-Directories -AllDirectories $allDirectories
